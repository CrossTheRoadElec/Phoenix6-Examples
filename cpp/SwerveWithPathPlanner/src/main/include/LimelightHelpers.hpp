...
namespace wpi::util {
    inline void from_json(const wpi::util::json& j, double& val)
    {
        val = j.get_number();
    }
    template <typename T>
    inline void from_json(const wpi::util::json& j, std::vector<T>& val)
    {
        auto const arr = j.get_array();
        for (auto const &v : arr) {
            val.emplace_back(v.template get<T>());
        }
    }
    inline void from_json(const wpi::util::json& j, std::string& val)
    {
        val = j.get_string();
    }
}
...
namespace LimelightHelpers
{
    ...
    inline wpi::math::Pose3d toPose3D(const std::vector<double>& inData)
    {
        ...
        return wpi::math::Pose3d(
            wpi::math::Translation3d(wpi::units::length::meter_t(inData[0]), wpi::units::length::meter_t(inData[1]), wpi::units::length::meter_t(inData[2])),
            wpi::math::Rotation3d(wpi::units::angle::degree_t(inData[3]), wpi::units::angle::degree_t(inData[4]),
                   wpi::units::angle::degree_t(inData[5])));
    }
    ...
    inline wpi::math::Pose2d toPose2D(const std::vector<double>& inData)
    {
        ...
        return wpi::math::Pose2d(
            wpi::math::Translation2d(wpi::units::length::meter_t(inData[0]), wpi::units::length::meter_t(inData[1])), 
            wpi::math::Rotation2d(wpi::units::angle::degree_t(inData[5])));
    }
    ...
    {
        wpi::nt::NetworkTableEntry entry = LimelightHelpers::getLimelightNTTableEntry(limelightName, "rawfiducials");
    ...
    {
        wpi::nt::NetworkTableEntry entry = LimelightHelpers::getLimelightNTTableEntry(limelightName, "rawdetections");
    ...
    public:
        ...
        PoseEstimate() = default;
         ...
         PoseEstimate(const wpi::math::Pose2d& pose, wpi::units::time::second_t timestampSeconds, 
    ...
    inline std::optional<PoseEstimate> getBotPoseEstimate(const std::string& limelightName, const std::string& entryName) {
        wpi::nt::NetworkTableEntry poseEntry = getLimelightNTTableEntry(limelightName, entryName);
        ...
        wpi::units::time::second_t timestamp = wpi::units::time::second_t((poseEntry.GetLastChange() / 1000000.0) - (latency / 1000.0));
