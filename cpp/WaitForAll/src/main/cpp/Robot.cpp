...
void Robot::RobotPeriodic() {
  ...
  /* If we press the A button, test what happens when we wait on lots of signals (normal use case) */
    if(m_joystick.GetAButtonPressed()) {
      ctre::phoenix::StatusCode status = ctre::phoenixpro::BaseStatusSignalValue::WaitForAll(m_waitForAllTimeout, m_lotsOfSignals);
    ...
    if(m_joystick.GetBButtonPressed()) {
      ctre::phoenix::StatusCode status = ctre::phoenixpro::BaseStatusSignalValue::WaitForAll(m_waitForAllTimeout, m_signalsAcrossCANbuses);
    ...
    if(m_joystick.GetYButtonPressed()) {
      ctre::phoenix::StatusCode status = ctre::phoenixpro::BaseStatusSignalValue::WaitForAll(m_waitForAllTimeout, m_noSignals);
    ...
    if(m_joystick.GetXButtonPressed()) {
      ctre::phoenix::StatusCode status = ctre::phoenixpro::BaseStatusSignalValue::WaitForAll(m_waitForAllTimeout, {&m_canbus1signal1,
    &m_canbus1signal2,
