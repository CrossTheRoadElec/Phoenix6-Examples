...
public class Robot extends TimedRobot {
    ...
    private final CANcoder rightSensor = new CANcoder(2, CANBUS);
    private final Pigeon2 imu = new Pigeon2(0, CANBUS);
    ...
    private final DifferentialDrive drivetrain = new DifferentialDrive(leftFX::setThrottle, rightFX::setThrottle);
    ...
    public void simulationPeriodic() {
        ...
        final var leftPos = metersToRotations(
            Meters.of(m_driveSim.getLeftPosition())
        );
        // This is OK, since the time base is the same
        final var leftVel = metersToRotationsVel(
            MetersPerSecond.of(m_driveSim.getLeftVelocity()))
        ;
        final var rightPos = metersToRotations(
            Meters.of(m_driveSim.getRightPosition())
        );
        // This is OK, since the time base is the same
        final var rightVel = metersToRotationsVel(
            MetersPerSecond.of(m_driveSim.getRightVelocity())
        );
