...
public class DriveSubsystem extends SubsystemBase {
    ...
    public void simulationPeriodic() {
        ...
        m_leftSimState.setRawRotorPosition(
            metersToRotations(Meters.of(m_driveSim.getLeftPosition()))
        );
        m_leftSimState.setRotorVelocity(
            // This is OK, since the time base is the same
            metersToRotationsVel(MetersPerSecond.of(m_driveSim.getLeftVelocity()))
        );
        m_rightSimState.setRawRotorPosition(
            metersToRotations(Meters.of(m_driveSim.getRightPosition()))
        );
        m_rightSimState.setRotorVelocity(
            // This is OK, since the time base is the same
            metersToRotationsVel(MetersPerSecond.of(m_driveSim.getRightVelocity()))
        );
